...
namespace Trio
{
    class Pwd
    {
        ...
        public static bool CreateSubKey()
        {
            try
            {
                RegistryKey sk = key.CreateSubKey(@"Trio");
                return true;
            }
            catch (Exception ex)
            {
        ...
        public static bool SetValue(string username, string password)
        {
            try
            {
                RegistryKey SK_Key = key.OpenSubKey(@"Trio", true);
        ...
        public static bool GetKey()
        {
            ...
            {
                users.Clear();
                RegistryKey SK_Key = key.OpenSubKey(@"Trio");
        ...
        public static string Encrypt(string encryptStr, string key = "Hwa won't failinthecombiMathexam")
        {
        ...
        public static string Decrypt(string decryptStr, string key = "Hwa won't failinthecombiMathexam")
        {
            byte[] keyArray = Encoding.UTF8.GetBytes(key);
